...
import { ArrowLeft, DollarSign, Ticket, Users, TrendingUp, ChevronDown, ChevronUp } from "lucide-react";
...
const Dashboard = () => {
  ...
  // Fetch order items for organizer's events
  const { data: orderItems } = useQuery({
    queryKey: ["dashboard-order-items", user?.id],
  ...
  // Fetch related orders for attendee info
  const { data: orders } = useQuery({
    queryKey: ["dashboard-orders", user?.id, orderItems],
  ...
  // Chart data
  const chartData = eventStats
    .filter((s) => s.revenue > 0 || s.tickets > 0)
    .slice(0, 8)
    .map((s) => ({
      name: s.event.title.length > 20 ? s.event.title.slice(0, 18) + "…" : s.event.title,
      revenue: s.revenue,
      tickets: s.tickets,
    }));
      ...
      <div className="container max-w-5xl py-10">
        ...
        {eventStats.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            <p>No events yet.</p>
            <Button variant="hero" size="sm" className="rounded-full mt-4" asChild>
                ...
                {expandedEvent === event.id && (
                  <div className="border-t px-5 py-4">
                    {eventOrders.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No orders yet for this event.</p>
                    ) : (
                      <>
                            ...
                            <tbody>
                              {eventOrders.map((order) => (
                                <tr key={order.id} className="border-b last:border-0">
                                  ...
                                  <td className="py-2 pr-4 text-muted-foreground">{order.customer_email}</td>
